...
class WUIBody {
	...
	#importSections(sections, target, auto, nested) {
		sections.forEach(sec => {
			const enabled = typeof sec.enabled === "function" ? sec.enabled() : sec.enabled;
			if (typeof sec.enabled === "undefined" || enabled) {
				...
				if (!nested) {
					const tag = !auto ? (sec.tag || "div").toLowerCase() : sec.id.match(/section$/i) ? "section" : sec.id.match(/form$/i) ? "form" : "div";
