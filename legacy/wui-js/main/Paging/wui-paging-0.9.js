...
class WUIPaging {
	...
	#getDelay() {
		const time = getComputedStyle(this.#htmlElement || document.body).getPropertyValue("--wui-paging-page-transition-time");
	...
	select(target, onChange = this.onChange, instant = false) {
		...
		if (this.index !== index) {
			...
			const delay = instant ? 0 : this.#getDelay();
			if (instant) {
				inputPage.style.transition = "none";
				outputPage.style.transition = "none";
			}
			...
			setTimeout(() => {
				...
				if (instant) {
					inputPage.style.removeProperty("transition");
					outputPage.style.removeProperty("transition");
				}
				this.getPages().forEach((page, i) => {
