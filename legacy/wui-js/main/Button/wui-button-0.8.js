...
class WUIButton {
		...
		if (typeof (value) == "string" && value != "") {
			...
			if (this.#htmlElement instanceof HTMLButtonElement) {
				if (this.#htmlElement.hasChildNodes()) {
					const div = this.#htmlElement.querySelector("div");
					const img = this.#htmlElement.querySelector("img");
					const span = this.#htmlElement.querySelector("span");
					if (div) this.#properties.iconClass = div.className;
					if (img) this.#properties.iconImage = img.src;
					if (span) {
						this.#properties.textClass = span.className;
						this.#properties.text = span.innerHTML;
						this.#properties.textData = [];
						Array.from(span.attributes).forEach(({ name, value }) => {
							if (name.startsWith("data-")) this.#properties.textData.push([name.substring(5), value]);
						});
					}
					this.#htmlBuilt = true;
				}
	...
	#buildHTML() {
		if (!(this.#htmlElement instanceof HTMLButtonElement) || this.#htmlBuilt || !this.#initialized) return;
		let html = "";
		...
		if (this.#properties.textClass != null) {
			let dataAttrs = "";
			if (this.#properties.textData) {
				Object.entries(this.#properties.textData).forEach(([key, value]) => {
		...
		if (this.#htmlElement instanceof HTMLButtonElement) {
			...
			this.#htmlElement.addEventListener("click", () => {
				...
				if (!this.#properties.locked && this.#properties.enabled && typeof (this.#properties.onClick) == "function") {
					const selected = this.#properties.selectable ? this.#htmlElement.classList.contains("selected") : null;
					this.#properties.onClick(selected);
				}
			});
			this.#htmlElement.addEventListener("dblclick", () => {
				...
				if (!this.#properties.locked && this.#properties.enabled && typeof (this.#properties.onDblClick) == "function") {
					const selected = this.#properties.selectable ? this.#htmlElement.classList.contains("selected") : null;
					this.#properties.onDblClick(selected);
				}
			});
			this.#setStyle();
		}
