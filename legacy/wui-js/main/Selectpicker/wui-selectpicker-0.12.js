...
class WUISelectpicker {
	...
	open() {
		...
		if ((this.hidden || opener instanceof HTMLDivElement) && overlay instanceof HTMLDivElement && box instanceof HTMLDivElement) {
			...
			const elementPositionY = mobile || this.hidden ? 0 : this.openDirection === "up" ? window.innerHeight - elementRect.top : elementRect.bottom;
			const optionsPadding = mobile || this.hidden ? 0 : 10;
			const optionHeight = mobile || this.hidden ? 44 : 30;
			const optionBorder = mobile || this.hidden ? 1 : 0;
			const footerHeight = mobile || this.hidden ? 52 : 0;
			const optionsMaxHeight = () => {
				return maxOptions * optionHeight - optionBorder + optionsPadding;
			};
			let maxOptions = 11;
			...
			while (optionsMaxHeight() > window.innerHeight - elementPositionY - footerHeight) {
				maxOptions--;
			}
