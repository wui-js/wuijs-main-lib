...
class WUIMenubar {
	...
	get nowrapTooltips() {
		return this.#properties.nowrapTooltips;
	}
	...
	set nowrapTooltips(value) {
		if (typeof (value) === "boolean") {
			this.#properties.nowrapTooltips = value;
		}
	}
	...
	init() {
		...
		if (htmlElement instanceof HTMLElement) {
			const { bar, barHeader, barTop, barMain, barBottom, submenu, submenuHeader, submenuMain } = this.#htmlElements;
			const barSeparationTop = document.createElement("div");
			const barSeparationBottom = document.createElement("div");
			...
			bar.append(barTop);
			bar.append(barSeparationTop);
			bar.append(barMain);
			bar.append(barSeparationBottom);
			bar.append(barBottom);
			barHeader.className = "header";
			if (!this.expansive) {
				barHeader.style.display = "none";
			}
			barTop.className = "top";
			if (this.topButtons.length === 0) {
				barTop.style.display = "none";
			}
			barSeparationTop.className = "separation";
			if (!this.separations || this.topButtons.length === 0 || this.mainButtons.length === 0) {
				barSeparationTop.style.display = "none";
			}
			barMain.className = "main";
			if (this.mainButtons.length === 0) {
				barMain.style.display = "none";
			}
			barSeparationBottom.className = "separation";
			if (!this.separations || this.topButtons.length === 0 || this.bottomButtons.length === 0) {
				barSeparationBottom.style.display = "none";
			}
			barBottom.className = "bottom";
	...
	#addButton(options) {
		...
		text.className = "text";
		tooltip.className = "tooltip" + (!options.label || (typeof (options.tooltipable) === "boolean" && !options.tooltipable) ? " hidden" : "") + (this.nowrapTooltips ? " nowrap" : "");
	...
	setPhoto(id = "", src = "") {
		if (id !== "") {
			...
			const options = this.getButtonOptions(id);
			if (options instanceof Object) {
				options.photoImage = src;
			}
					...
					if (typeof (options.radioMode) === "boolean" && !options.radioMode) {
						...
						this.#buttons.filter(opt => opt.id !== id && opt.id !== parentId && opt.parentId !== id).forEach(opt => {
							...
							if (btn instanceof HTMLElement && !btn.classList.contains("disabled")) {
								...
								this.getButtonOptions(opt.id).selected = false;
							}
						});
						button.classList.add("selected");
						if (hasChildren) {
	...
	#open(id) {
		...
		const options = this.getButtonOptions(id);
		const buttons = options instanceof Object ? options.buttons || [] : [];
...
/*
HTML output:
<div class="wui-menubar[ fixed][ centered][ compacted]">
	<div class="bar">
		<div class="header">
			<div class="expander">
				<div class="icon"></div>
