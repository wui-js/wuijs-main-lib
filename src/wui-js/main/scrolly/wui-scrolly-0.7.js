...
class WUIScrolly {
	...
	init() {
		...
		const onScrollJS = (event) => {
			if (!this.#lock) {
				this.#properties.direction = window.scrollY > this.scrollY ? "down" : window.scrollY < this.scrollY ? "up" : this.direction;
				...
				if (!this.#truncated && event.deltaY !== 0) {
					...
					if (this.sections.length > 0) {
						...
						if (y > 0) {
							...
							this.#properties.sceneProgress = this.deltaY > 0 ? 1 : 0;
						}
			...
			if (typeof (section.selector) === "string" && section.selector !== "") {
				...
				if (this.sections[i]._paging && typeof (section.pages) === "number" && section.pages > 0) {
					for (let j = 0; j < section.pages; j++) {
						...
						this.sections[i]._paging.appendChild(indicator);
					}
				}
			}
		}, this);
		document.documentElement.dataset[this.dataScrollY] = window.scrollY >= 0 ? window.scrollY : 0;
		this.#scrollCSS = debounce(onScrollCSS);
		const hasScrollJS = this.sections.length > 0 || (typeof (this.onStart) === "function" || typeof (this.onMove) === "function" || typeof (this.onStop) === "function");
		if (hasScrollJS) {
			this.#scrollJS = debounce(onScrollJS);
			this.#properties.sceneIndex = null;
			this.#properties.sceneStep = null;
			this.#properties.sceneProgress = null;
		}
		["scroll", "touchmove"].forEach(type => {
			document.addEventListener(type, this.#scrollCSS, { passive: true });
			if (this.#scrollJS) {
				document.addEventListener(type, this.#scrollJS, { passive: false });
			}
		}, this);
	}
	...
	destroy() {
		if (this.#scrollCSS) {
			document.removeEventListener("scroll", this.#scrollCSS);
			document.removeEventListener("touchmove", this.#scrollCSS);
			this.#scrollCSS = undefined;
		}
		if (this.#scrollJS) {
			document.removeEventListener("scroll", this.#scrollJS);
			document.removeEventListener("touchmove", this.#scrollJS);
			this.#scrollJS = undefined;
		}
		if (this.#stop) {
			clearTimeout(this.#stop);
		}
