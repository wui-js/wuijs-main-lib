...
class WUIModal {
	...
	resposive() {
		...
		if (box instanceof HTMLElement && page) {
			...
			box.style.top = mobile && !small ? "calc(44px + " + mobileMargin + "px)" : slide ? slideMargin + "px" : small ? (bodyHeight - this.#boxHeight) + "px" : "auto";
			...
			box.style.height = (mobile && !small) || slide ? "auto" : "var(--wui-modal-" + (small ? "small" : "") + "page-box-height)";
		}
	}
	...
	open(onOpen = this.onOpen, delay = this.openDelay) {
		...
		if (under instanceof WUIModal && page && !small && underPage && !underSmall) {
		...
		this.#animationInterval = setInterval(() => {
			...
			if (under instanceof WUIModal && page && !small && underPage && !underSmall) {
				const underHtmlElement = under.#htmlElement;
				const underSlide = Boolean(underHtmlElement.classList.contains("slide"));
				const underMaximized = Boolean(underHtmlElement.classList.contains("maximized"));
				const { overlay: underOverlay, box: underBox } = under.#htmlElements;
				if (underOverlay instanceof HTMLElement) {
					underOverlay.style.opacity = (1 - ease);
				}
				if (underBox instanceof HTMLElement) {
					if (!mobile && underSlide) {
						// ...
					} else if (mobile && !underMaximized) {
						underBox.style.top = (mobileMargin + 44 * (1 - 1.5 * ease)) + "px";
