...
class WUISelectpicker {
		...
		if (htmlElement instanceof HTMLDivElement) {
			...
			if (!this.hidden) {
				...
				if (!this.#htmlElements.inputIcon) {
					...
					htmlElement.appendChild(this.#htmlElements.inputIcon);
				}
				if (!this.#htmlElements.inputText) {
					...
					htmlElement.appendChild(this.#htmlElements.inputText);
				}
			}
	...
	#getClipBoundary() {
		let node = this.#htmlElement.parentElement;
		let top = 0;
		let bottom = window.innerHeight;
		while (node instanceof HTMLElement) {
			const style = getComputedStyle(node);
			if (style.overflow.match(/(hidden|auto|scroll|clip)/) || style.overflowY.match(/(hidden|auto|scroll|clip)/)) {
	...
	#loadBox() {
		...
		Array.from(input.options).forEach((opt, i) => {
			...
			if (option instanceof HTMLDivElement && typeof (option.dataset.value) !== "undefined") {
				if (opt.selected) {
					const rawScrollTop = option.offsetTop - parseInt((options.clientHeight - option.clientHeight) / 2);
					options.scrollTop = Math.round(rawScrollTop / option.clientHeight) * option.clientHeight;
	...
	open() {
		const mobile = Boolean(window.matchMedia("(max-width: 767px)").matches);
		const htmlElement = this.#htmlElement;
		const { input, opener, overlay, box, options } = this.#htmlElements;
		if ((this.hidden || opener instanceof HTMLDivElement) && overlay instanceof HTMLDivElement && box instanceof HTMLDivElement) {
			...
			const clipBoundary = mobile || this.hidden ? null : this.#getClipBoundary();
			const availableSpace = mobile || this.hidden ? window.innerHeight : this.openDirection === "up" ? elementRect.top - clipBoundary.top : clipBoundary.bottom - elementRect.bottom;
			const optionsPadding = mobile || this.hidden ? 0 : 10;
			const optionHeight = mobile || this.hidden ? 44 : 30;
			const footerHeight = mobile || this.hidden ? 52 : 0;
			const boxBorder = mobile || this.hidden ? 0 : 2;
			const optionsMaxHeight = () => {
				return maxOptions * optionHeight + optionsPadding;
			};
			let maxOptions = 11;
			...
			while (optionsMaxHeight() + boxBorder > availableSpace - footerHeight) {
				maxOptions--;
			}
			overlay.style.zIndex = 101;
			overlay.classList.remove("hidden");
			box.className = `box ${this.boxAlign} ${this.openDirection}`;
			box.style.marginBottom = !mobile && !this.hidden && this.openDirection === "up" ? htmlElement.clientHeight + "px" : "auto";
