...
class WUIFormat {
	...
	static #data = {
		tlds: ("" // http://data.iana.org/TLD/tlds-alpha-by-domain.txt, 20260602
				...
				+ " RACING RADIO RE READ REALESTATE REALTOR REALTY RECIPES RED REDUMBRELLA REHAB REISE REISEN REIT RELIANCE REN RENT RENTALS REPAIR REPORT REPUBLICAN REST RESTAURANT REVIEW REVIEWS REXROTH RICH RICHARDLI RICOH RIL RIO RIP RO ROCKS RODEO ROGERS ROOM RS RSVP RU RUGBY RUHR RUN RW RWE RYUKYU"
				...
				+ " WALES WALMART WALTER WANG WANGGOU WATCH WATCHES WEATHER WEATHERCHANNEL WEBCAM WEBER WEBSITE WED WEDDING WEIBO WEIR WF WHOSWHO WIEN WIKI WILLIAMHILL WIN WINDOWS WINE WINNERS WME WOODSIDE WORK WORKS WORLD WOW WS WTC WTF"
		...
		locales: ("" // https://gist.github.com/eddieoz/63d839c8a20ef508cfa4fa9562632a21 20250617
				+ " af af-ZA ar ar-AE ar-BH ar-DZ ar-EG ar-IQ ar-JO ar-KW ar-LB ar-LY ar-MA ar-OM ar-QA ar-SA ar-SY ar-TN ar-YE az az-AZ az-AZ"
				+ " be be-BY bg bg-BG bs-BA"
				+ " ca ca-ES cs cs-CZ cy cy-GB"
				+ " da da-DK de de-AT de-CH de-DE de-LI de-LU dv dv-MV"
				+ " el el-GR en en-AU en-BZ en-CA en-CB en-GB en-IE en-JM en-NZ en-PH en-TT en-US en-ZA en-ZW eo es es-AR es-BO es-CL es-CO es-CR es-DO es-EC es-ES es-ES es-GT es-HN es-MX es-NI es-PA es-PE es-PR es-PY es-SV es-UY es-VE et et-EE eu eu-ES"
				+ " fa fa-IR fi fi-FI fo fo-FO fr fr-BE fr-CA fr-CH fr-FR fr-LU fr-MC"
				+ " gl gl-ES gu gu-IN"
				+ " he he-IL hi hi-IN hr hr-BA hr-HR hu hu-HU hy hy-AM"
				...
				+ " ro ro-RO ru ru-RU"
				+ " sa sa-IN se se-FI se-FI se-FI se-NO se-NO se-NO se-SE se-SE se-SE sk sk-SK sl sl-SI sq sq-AL sr-BA sr-BA sr-SP sr-SP sv sv-FI sv-SE sw sw-KE syr syr-SY"
	...
	static #initNumber() {
		Number.prototype.wuiDefaults = Object.assign({}, WUIFormat.#defaults.number);
		Number.prototype.wuiSetDefaults = function (options = {}) {
			Object.entries(this.wuiDefaults).forEach(([name, value]) => {
				if (name in options) {
					this.wuiDefaults[name] = options[name];
				}
			});
			return this;
		}
		Number.prototype.wuiToString = function (options = {}) {
			...
			const prefix = options.numberPrefix;
			const thoSep = options.thousandsSeparator;
			const decSep = options.decimalSeparator;
			const sufix = options.numberSufix;
			let number = parseFloat(this);
			const symbol = number < 0 ? "-" : "";
			let decLen = options.decimalLength;
			let i, j;
	...
	static #initString() {
		String.prototype.wuiDefaults = Object.assign({}, WUIFormat.#defaults.string);
		String.prototype.wuiSetDefaults = function (options = {}) {
			Object.entries(this.wuiDefaults).forEach(([name, value]) => {
				if (name in options) {
					this.wuiDefaults[name] = options[name];
				}
		...
		String.prototype.wuiValidateEmail = function () {
			...
			return /^\w[\w\-\.\~\+]*\@\w[\w\-\.]*\.\w{2,18}$/.test(this) && tld != null && WUIFormat.#data.tlds.indexOf(tld) > -1 ? true : false;
		}
		...
		String.prototype.wuiValidateUrl = function () {
			...
			return regexp.test(this) && tld !== "" && WUIFormat.#data.tlds.indexOf(tld) > -1 ? true : false;
		}
		String.prototype.wuiValidateUrlList = function (separator = this.wuiDefaults.urlListSeparator) {
			const list = this.split(separator);
			let valid = this !== null && this !== "";
			for (let i in list) {
				if (!list[i].toString().trim().wuiValidateUrl()) {
					valid = false;
				}
			}
			return valid;
	...
	static #initDate() {
		Date.prototype.wuiDefaults = Object.assign({
			_daysNames: [],
			_monthsNames: []
		}, WUIFormat.#defaults.date);
		Date.prototype.wuiSetDefaults = function (options = {}) {
			const locales = this.wuiDefaults.locales;
			Object.entries(this.wuiDefaults).forEach(([name, value]) => {
				if (name in options) {
					this.wuiDefaults[name] = options[name];
				}
			});
			if (options.locales && options.locales !== locales) {
				Date.prototype.wuiLoadNames();
			}
			return this;
		}
		Date.prototype.wuiLoadNames = function (locales = this.wuiDefaults.locales) {
			if (WUIFormat.#data.locales.indexOf(locales.toLowerCase()) > -1) {
				let i;
				for (i = 0; i < 7; i++) {
					const name = new Date(2023, 0, i + 1).toLocaleString(locales, { weekday: "long" }); // 2023-01-01: sunday
					this.wuiDefaults._daysNames[i] = name.replace(/^\s*(\w)/, letter => letter.toUpperCase());
				}
				for (i = 0; i < 12; i++) {
		...
		Date.prototype.wuiToString = function (format = "default", options = {}) {
			...
			const year = utc ? this.getUTCFullYear() : this.getFullYear(),
				...
				weekDayName = options._daysNames[weekDay],
		...
		Date.prototype.wuiDayName = function (day = this.getDay()) {
			return typeof (day) === "number" && day >= 0 && day <= 6 ? this.wuiDefaults._daysNames[day] : "error";
		}
		Date.prototype.wuiMonthName = function (month = this.getMonth()) {
			return typeof (month) === "number" && month >= 0 && month <= 11 ? this.wuiDefaults._monthsNames[month] : "error";
		}
	}

	static {
		WUIFormat.#initNumber();
		WUIFormat.#initString();
		WUIFormat.#initDate();
		Date.prototype.wuiLoadNames();
	}
